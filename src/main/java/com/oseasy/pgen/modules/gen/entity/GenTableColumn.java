...
public class GenTableColumn extends DataEntity<GenTableColumn> {
	...
	private String isUpdate;		// 是否支持单独修改
	private String isUpdatePl;		// 是否支持单独批量修改
    ...
    public String getIsUpdatePl() {
        return isUpdatePl;
    }

    public void setIsUpdatePl(String isUpdatePl) {
        this.isUpdatePl = isUpdatePl;
    }
	...
	public String getJavaFieldGid() {
	    return (StringUtil.substringBefore(getJavaField(), "|")).replace(".id", "().getId");
	}
	...
	public String getJavaFieldGname() {
	    String[][] ss = getJavaFieldAttrs();
	    return (ss.length>0 ? getSimpleJavaField()+"."+ss[0][0] : "").replace(".name", "().getName");
	}
