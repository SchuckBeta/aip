 ...
 *
 */
package com.oseasy.pcore.common.supcan;
...
import com.oseasy.pcore.common.supcan.annotation.treelist.SupTreeList;
import com.oseasy.pcore.common.supcan.annotation.treelist.cols.SupCol;
import com.oseasy.pcore.common.supcan.annotation.treelist.cols.SupGroup;
import com.oseasy.pcore.common.supcan.treelist.TreeList;
import com.oseasy.pcore.common.supcan.treelist.cols.Col;
import com.oseasy.pcore.common.supcan.treelist.cols.Group;
...
public class SupcanController extends BaseController {
	...
	public TreeList treeList(@PathVariable("typeAlias") String typeAlias) {
		...
		for (Method m : clazz.getMethods()) {
			...
			if (supCol != null) {
				...
				if (StringUtil.isBlank(col.getName())) {
					col.setName(StringUtil.uncapitalize(StringUtil.substring(m.getName(), 3)));
				}

				// 无分组
				if (StringUtil.isBlank(supCol.groupId())) {
					cols.add(col);
				}
				// 有分组
				else{
	...
	private void listToTree(List<Object> colList, Map<String, Group> groupMap, String parentId, Comparator<Object> comparator) {
		for (Map.Entry<String, Group> e : groupMap.entrySet()) {
			Group g = e.getValue();
			if (StringUtil.equals(parentId, g.getParentId())) {
				colList.add(g);
				// 判断是否有子节点，有的话则加进去
