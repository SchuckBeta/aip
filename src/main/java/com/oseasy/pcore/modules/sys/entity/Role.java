 ...
 */
public class Role extends DataEntity<Role> implements IidEntity{
	...
	private String bizType;//角色业务类型 字典值
	private String dataScope;// 数据范围

	private String roleGroup  ;//前台后台角色 1:前台 2:后台
...
//	}
	...
	public int compareTo(Role o2) {
		if(this == null || o2 == null){
			return -1;
		}
		if(this.getId().length() > o2.getId().length()){
			return 1;
		}
		if(this.getId().length() < o2.getId().length()){
			return -1;
		}
		if(this.getId().compareTo(o2.getId()) > 0){
			return 1;
		}
		if(this.getId().compareTo(o2.getId()) < 0){
			return -1;
