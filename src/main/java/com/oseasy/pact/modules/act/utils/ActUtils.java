...
import com.oseasy.pcore.common.annotation.FieldName;
...
//	}
	...
	public static Map<String, Object> getMobileEntity(Object entity,String spiltType) {
			...
			for (Method m : entity.getClass().getMethods()) {
				if (m.getAnnotation(JsonIgnore.class) == null && m.getAnnotation(JsonBackReference.class) == null && m.getName().startsWith("get")) {
					...
					if (m.getName().equals("getAct")) {
						...
						map.put("taskId", ObjectUtil.toString(m.invoke(act, new Object[]{}), ""));
					}else{
						field.add(StringUtil.uncapitalize(m.getName().substring(3)));
						value.add(ObjectUtil.toString(m.invoke(entity, new Object[]{}), ""));
					}
				}
			}
		}catch (Exception e) {
		...
		map.put("beanTitles", StringUtil.join(field, spiltType));
		map.put("beanInfos", StringUtil.join(value, spiltType));
		map.put("chineseNames", StringUtil.join(chinesName, spiltType));
	...
	public static UserEntity toActivitiUser(User user) {
		...
		userEntity.setLastName(StringUtil.EMPTY);
