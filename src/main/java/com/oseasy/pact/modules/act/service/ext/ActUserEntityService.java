...
//		return (UserEntity) getDbSqlSession().selectOne("selectUserById", userId);
		return ActUtils.toActivitiUser(getCoreService().getUserByLoginName(userId));
	}
...
//		}
		...
		if (user != null) {
		    getCoreService().deleteUser(new com.oseasy.pcore.modules.sys.entity.User(user.getId()));
		}
	}
...
//		return getDbSqlSession().selectList("selectGroupsByUserId", userId);
		...
		for (Role role : getCoreService().findRole(new Role(new com.oseasy.pcore.modules.sys.entity.User(null, userId)))) {
