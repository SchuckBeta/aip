...
public class ActProModel implements IActProDeal {
	//@Autowired
...
//	CategoryService categoryService = (CategoryService) SpringContextHolder.getBean(CategoryService.class);
	...
	public Boolean dealMenu(ActProParamVo actProParamVo) {
		...
		if (proProject != null) {
//			Menu  menu= coreService.getMenu(SysIds.SITE_MENU_PROJECT_ROOT.getId());
			Menu  menu = null;
				...
				if ((sourcelist != null) && (sourcelist.size() > 0)) {
					for (int i = 0; i < sourcelist.size(); i++) {
						...
						if (actYwGnodeIndex != null) {
							...
							menuNextForm.setRemarks(ActMenuRemarks.XM.getName());
...
//				}
	...
	public Boolean dealCategory(ActProParamVo actProParamVo) {
		...
		ActYw actYw =actProParamVo.getActYw();
		//修改栏目为可见
		//Category category = categoryService.get(SysIds.SITE_CATEGORYS_PROJECT_ROOT.getId());
		String siteId=(String)CoreUtils.getCache("siteId");
//		Category category =categoryService.getByIdAndType(siteId,"0000000283");
//		if(category!=null){
//			//默认添加申报表单
