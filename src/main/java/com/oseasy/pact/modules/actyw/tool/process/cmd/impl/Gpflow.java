 ...
 *
 */
public class Gpflow extends ActYwAbsEngine<ActYwEngineImpl> implements ActYwCommand<ActYwPgnode>, ActYwCommandCheck<ActYwPgnode, ActYwRgnode, ActYwGnode>{
    protected static final Logger logger = Logger.getLogger(Gpflow.class);

    @Override
    public ActYwEcmd cmd() {
        return ActYwEcmd.ECMD_P_FLOW;
    }
    ...
    public Gpflow(ActYwEngineImpl engine) {
      super(engine);
    }
...
//        }
//
//        if(!((GnodeType.GT_PROCESS_START.getId()).equals(curPreGnode.getType()) ||
//                (GnodeType.GT_PROCESS_GATEWAY.getId()).equals(curPreGnode.getType()) ||
//                (GnodeType.GT_PROCESS_TASK.getId()).equals(curPreGnode.getType()))){
//            tpl = new ActYwPgnode();
...
//            return tpl;
//        }else{
//            if((GnodeType.GT_PROCESS_GATEWAY.getId()).equals(curPreGnode.getType())){
...
//        }
//
//        if(!((GnodeType.GT_PROCESS_START.getId()).equals(curPreFunGnode.getType()) ||
//                (GnodeType.GT_PROCESS_GATEWAY.getId()).equals(curPreFunGnode.getType()) ||
//                (GnodeType.GT_PROCESS_TASK.getId()).equals(curPreFunGnode.getType()))){
//            tpl = new ActYwPgnode();
