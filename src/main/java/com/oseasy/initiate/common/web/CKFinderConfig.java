...
public class CKFinderConfig extends Configuration {
  ...
  protected Configuration createConfigurationInstance() {
    Principal principal = (Principal) CoreUtils.getPrincipal();
    if (principal == null) {
    ...
    try {
      ...
      this.baseURL = FileUtil.path(
          Servlets.getRequest().getContextPath() + Global.USERFILES_BASE_URL + principal + "/");
      this.baseDir = FileUtil
          .path(Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + principal + "/");
    } catch (Exception e) {
  ...
  public boolean checkAuthentication(final HttpServletRequest request) {
    return CoreUtils.getPrincipal() != null;
  }
