 ...
 *
 */
package com.oseasy.pcas.modules.cas.security;
...
import com.oseasy.pcore.modules.sys.security.SmsCredentialsMatcher;
 ...
 * @version 2014-7-5
 */
@Service("frontCasRealm")
public class FrontCasRealm extends SystemAuthorizingRealm {
...
//		if (!token.isCas()) {
//     	   throw new AuthenticationException("msg:Cas单点登录错误, 请重试.");
//        }

		// 校验用户名密码
		User user = getCoreService().getUserByLoginName(token.getUsername());
		if (user != null) {
//	    if (user != null&&(CoreUtils.checkHasRole(user, RoleBizTypeEnum.XS.getValue()) || CoreUtils.checkHasRole(user, RoleBizTypeEnum.DS.getValue()))) {
			...
			throw new AuthenticationException("msg:该登录名不存在，请注册.");
		}
	}
	...
	public void initCredentialsMatcher() {
		SmsCredentialsMatcher matcher = new SmsCredentialsMatcher();
