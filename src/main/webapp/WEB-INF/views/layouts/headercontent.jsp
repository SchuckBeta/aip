...
<link rel="stylesheet" type="text/css" href="/common/common-css/notify.css">
<style>
    .footer-notify-container .notify-body{
        box-sizing: border-box;
    }
    .footer-notify-container .btn-group button.btn + button{
        margin-left: 8px;
    }
    .notify-body .btn-accept,.notify-body .btn-refuse ,.notify-body .btn-view{
    ...
    .footer-notify-container .carousel{
        margin-bottom: 0;
    }
</style>
...
<div id="notifyModule" class="footer-notify-container">
    <div class="notify-wrap">
        <div class="notify-header clearfix">
            <span>消息内容<span class="no-readbar">（<span class="no-read-num">0</span>条）</span></span>
...
<script type="text/javascript">
    ...
    $(function () {
        $('li.remove-nav-index a').on('click',function () {

            if($(this).parent().hasClass('logout')){
                $.removeCookie('isShowOnce',{
                    expires: -1,
                    path: '/'
                })
            }
...
//            }
        ...
        NotifyModule.prototype.updateItem = function (data) {
            ...
            if (btns && btns.length > 0) {
                ...
                $.each(btns, function (i, item) {
                    itemHtml.push('<button type="button" type="button" class="btn btn-small" onClick="' + item.click + '">' + item.name + '</button>');
                });
        ...
        NotifyModule.prototype.tplListHtml = function (list) {
            ...
            $.each(list, function (i, item) {
                ...
                html += '<div class="item text-center ' + (i === 0 ? 'active' : '') + '" data-notify-id="' + item.notifyId + '" data-team-id="' + item.teamId + '"> <div class="cell">' +
...
//            this.btnView.off();
    ...
    function changeUnreadTrForNotify(nid) {
        ...
        $.ajax({
            type: 'post',
            url: '/a/oa/oaNotify/getReadFlag?'+Math.random(),
            data: {
                oaNotifyId: nid
            },
            success: function (data) {
                if (data == "1") {
                    var ntr = $("tr[key_5f820f675bfe4ffbb2b18f49ae12cbfe='" + nid + "']",$("#mainFrame").contents());
                    if (ntr && ntr.length > 0) {
	...
	function getUnreadCountForHead(){
		$.ajax({
			type:'post',
			url:'/a/oa/oaNotify/getUnreadCount?'+Math.random(),
			dataType: "json",
			success:function(data){
