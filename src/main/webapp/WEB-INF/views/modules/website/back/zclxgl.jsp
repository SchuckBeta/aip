...
<div id="enterPersonManage" class="container-fluid container-fluid-oe">
    ...
    <div class="form-box">
        <form>
            <div class="text-right">
                <a href="javascript: void(0);" class="btn btn-back-oe btn-primaryBack-oe">创建资产类型</a>
                <button type="button" class="btn btn-back-oe btn-primaryBack-oe">删除资产类型</button>
            </div>
        </form>
    </div>
    <table class="table table-hover table-bordered table-condensed table-center table-theme-default">
        ...
        <tr v-for="(item,index) in list">
            ...
            <td>{{item.assetTypeNumber}}</td>
            <td>{{item.assetTypeLabel}}</td>
            <td>{{item.assetTypeCount}}</td>
            <td>{{item.assetDirtyNo}}</td>
            <td><span style="display: inline-block;vertical-align: middle;min-width: 350px;">{{item.assetRemark}}</span>
            </td>
            <td>
