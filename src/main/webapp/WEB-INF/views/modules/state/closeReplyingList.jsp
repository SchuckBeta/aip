    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#sub_list_wrap', window.parent.document).find('li a').each(function(){
                ...
                if(hrefReg.indexOf("closeReplyingList")!=-1){
                    if(count > 0){
                        if(count>99){
                            //count="99+";
                        }
            ...
            //当答辩评分审核后，左侧结果评定的数值会改变
            var message = "${message}";
            if(message=="答辩评分保存成功"){
                $('#sub_list_wrap', window.parent.document).find('li a').each(function(){
                    var hrefReg = $(this).attr('href');
                    var count = "${pj:assessCount()}";
...
<div class="container-fluid">
    ...
    <form:form id="searchForm" modelAttribute="projectDeclare" action="${ctx}/state/closeReplyingList" method="post"
    ...
    <table id="contentTable" class="table table-bordered table-condensed table-hover table-center table-orange table-nowrap table-sort">
        <thead>
        <tr>
        ...
        <c:forEach items="${page.list}" var="proDeclare">
            <tr>
                ...
                <td><a href="${ctx}/state/infoView?id=${proDeclare.id}&taskDefinitionKey=${proDeclare.act.histTask.taskDefinitionKey}">${proDeclare.name}</a></td>
                <td>
                ...
                <td>
                    <c:if test="${pj:getById(proDeclare.id).replyScore>0}">
                        ${pj:getById(proDeclare.id).replyScore}
                    </c:if>
                </td>
                <td>
                        ${proDeclare.year}
