...
<form:form id="inputForm" modelAttribute="projectDeclare" action="${ctx}/state/expCheckSave" method="post" >
	<form:hidden path="id"/>
		...
		<section class="row">
			...
			<div style="padding:10px" id="lastCheckToggle_wrap">
				<c:if test="${standardList!=null&&fn:length(standardList)>0}">
					...
					<div class="form-horizontal row" novalidate>
						<div class="form-group col-sm-6 col-md-6 col-lg-6">
							<label class="col-xs-4 " style="width: 150px" >中期检查评分：</label>
							<c:if test="${fn:length(standardList)==0}">
								<input  name="midScore" id="totalScore"  class="col-xs-5" style="width: 80px"    />
							</c:if>
							<c:if test="${fn:length(standardList)>0}">
								<input  name="midScore" id="totalScore" readonly  class="col-xs-5" style="width: 80px;"    />
							</c:if>
						</div>
					</div>
				...
				<div class="row" style="text-align: center; margin-bottom: 60px;">
					...
					<button type="button" class="btn btn-sm btn-primary" id="btnSumbit" onclick="submitExp();" style="margin-right: 50px;">提交</button>
