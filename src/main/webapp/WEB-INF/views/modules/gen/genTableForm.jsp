					...
					<div class="control-group">
						<table id="contentTable" class="table  table-bordered table-condensed">
							<thead><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th><th title="实体对象的属性字段类型">Java类型</th>
								...
								<th title="是否支持独立更新">更新</th><th title="是否支持独立批量更新">批量更新</th>
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<select name="columnList[${vs.index}].javaType" class="required input-mini" style="width:155px;*width:155px">
											<c:forEach items="${config.javaTypeList}" var="dict">
												<option value="${dict.value}" ${dict.value==column.javaType?'selected':''} title="${dict.description}">${dict.label}-${dict.description}</option>
											</c:forEach>
										</select>
									</td>
									<td>
									...
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isUpdate" value="1" ${column.isUpdate eq '1' ? 'checked' : ''}/>
									</td>
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isUpdatePl" value="1" ${column.isUpdatePl eq '1' ? 'checked' : ''}/>
									</td>
									<td>
									...
									<td>
										<select name="columnList[${vs.index}].queryType" class="required input-mini" style="width:90px;">
