    ...
    <form:form id="inputForm" modelAttribute="proModel" action="${ctx}/promodel/proModel/saveGcontestEdit"
        ...
        <div id="teamDetail" class="panel-body collapse in">
            <div class="panel-inner">
                ...
                <div class="control-group">
                    <label class="control-label"><i>*</i>所属领域：</label>
                    <div class="controls">
                        <input type="text" name="regionName"
                                   id="regionName" maxlength="64" value="${regionName}"
                                 readonly="readonly"  class="required"/>
                    </div>
                </div>

                <div class="control-group">
                    <label class="control-label"><i>*</i>领域分组：</label>
                    <div class="controls">
                        <input type="text" name="regionGroupName"
                               readonly="readonly" id="regionGroupName" maxlength="64" value="${regionGroupName}"
                                   class="required"/>
                    </div>
                </div>
        ...
        <c:if test="${not empty proModelMdGcHistoryList}">
            <c:forEach items="${proModelMdGcHistoryList}" var="proModelMdGcHistory">
                <div class="control-group">
                    <label class="control-label">
                        ${fns:getDictLabel(proModelMdGcHistory.type,'0000000169','')}获奖情况：</label>
                    <div class="controls">
                        <input type="hidden" name="proModelMdGcHistory.type" value="${proModelMdGcHistory.type}">
                        <input type="hidden" name="proModelMdGcHistory.id" value="${proModelMdGcHistory.id}">
                        <select name="proModelMdGcHistory.result" class="form-control has-disabled">
                            <c:forEach items="${fns:getDictList(proModelMdGcHistory.type)}" var="dict">
                                <option value="${dict.value}"
                                    <c:if test="${proModelMdGcHistory.result eq dict.value}">
                                        selected = "selected"
                                    </c:if>
                                >${dict.label}</option>
                            </c:forEach>
