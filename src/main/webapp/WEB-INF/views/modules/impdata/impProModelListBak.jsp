...
<head>
    ...
    <script src="/js/impdata/impProModelList.js"></script>
...
<div class="container-fluid">
    ...
    <form:form id="searchForm" action="${ctx}/impdata/promodellist" method="post"
        ...
        <table id="contentTable"
            ...
            <c:forEach items="${page.list}" var="data">
                ...
                <shiro:hasPermission name="sys:user:import">
                    <td>
                        <c:if test="${data.isComplete!=1}">
                            <a style="display: none" href="${ctx}/impdata/expProModelData?id=${data.id}"
                               class="btn isComplete btn-primary btn-small">下载错误数据</a>
                        </c:if>
                        <c:if test="${data.isComplete==1}">
                        	<c:if test="${data.fail!='0'}">
                            <a href="${ctx}/impdata/expProModelData?id=${data.id}"
