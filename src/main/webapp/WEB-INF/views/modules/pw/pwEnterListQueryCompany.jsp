    ...
    <el-form :model="searchListForm" ref="searchListForm" size="mini"
        ...
        <div class="search-block_bar clearfix">
            <div class="search-btns">
                <el-button type="primary" size="mini" :disabled="disabled" @click.stop.prevent="exportCompanyFiles">
                    {{disabled? '导出中...' : '导出'}}
                </el-button>
                <el-button type="primary" size="mini" :disabled="disabled" @click.stop.prevent="confirmDelList">批量删除
                </el-button>
            </div>
...
<script type="text/javascript">
    ...
    new Vue({
        ...
        methods: {
            ...
            exportCompanyFiles: function () {
                var self = this;
//                this.disabled = true;
                location.href = '/a/pw/pwEnter/ajaxCompanyExp?' + Object.toURLSearchParams(this.searchListForm)
            },
            ...
            getPwEnterListBGSH: function () {
                var self = this;
                this.loading = true;
                this.$axios.get('/pw/pwEnter/ajaxCompanyList?' + Object.toURLSearchParams(this.searchListForm)).then(function (response) {
                    var data = response.data;
                    if (data.status === 1) {
