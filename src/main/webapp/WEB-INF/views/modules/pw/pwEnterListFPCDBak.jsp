...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.ajax({
                type: "GET",
                url: "${ctx}/pw/pwEnter/ajaxListFPCD",
                data: {
...
<div class="container-fluid">
    ...
    <form:form id="searchForm" modelAttribute="pwEnter" action="${ctx}/pw/pwEnter/listFPCD" method="post"
...
<%--                     <c:if test="${not empty pwEnter.eproject[0]}">${fns:getDictLabel(pwEnter.eproject[0].type, 'pw_enter_type', '')}/ </c:if> --%>
                   ...
                   <%-- <c:if test="${(empty pwEnter.eroom) || (empty pwEnter.eroom.id)}">
	                    <c:if test="${(pwEnter.status eq '3')}">
	                        <span class="primary-color">
		                        ...
		                        <c:if test="${(pwEnter.status ne '6') && (pwEnter.status ne '1')}">(待分配)</c:if>
	                        </span>
	                    </c:if>
                    ...
                    <c:if test="${(not empty pwEnter.eroom) && (not empty pwEnter.eroom.id)}">
                    	<c:if test="${(pwEnter.status eq '3')}">
	                        <span class="primary-color">
                    			${fns:getDictLabel(pwEnter.status, 'pw_enter_status', '')}
                    			<c:if test="${(pwEnter.status ne '6') && (pwEnter.status ne '1')}">(已分配)</c:if>
                    		</span>
                    	</c:if>
                    	<c:if test="${(pwEnter.status ne '3')}">
                    		${fns:getDictLabel(pwEnter.status, 'pw_enter_status', '')}
                ...
                <shiro:hasPermission name="pw:pwEnter:edit">
                    <td>
                        <%-- <c:if test="${(empty pwEnter.eroom) || (empty pwEnter.eroom.id)}"> --%>
                       	<%-- <c:if test="${(pwEnter.status eq 1) || (pwEnter.status eq 3) || (pwEnter.status eq 4) || (pwEnter.status eq 7)}"> --%>
                            <a class="btn-primary btn-small btn" href="${ctx}/pw/pwEnterRoom/assignRoomForm?pwEnter.id=${pwEnter.id}&secondName=分配场地">分配场地</a>
                            <%-- <a class="check_btn btn-pray btn-lx-primary" href="${ctx}/pw/pwEnter/form?id=${pwEnter.id}">分配场地</a> --%>
                        <%-- </c:if> --%>
