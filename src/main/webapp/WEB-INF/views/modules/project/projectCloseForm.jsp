...
<body>
<form action="/f/project/projectClose/save" id="inputForm" method="post">
    <div class="top-title">
            ...
            <div class="row content-wrap">
                    ...
                    <div class="table-wrap">
                        <table class="table  table-hover table-bordered">
                            ...
                            <c:forEach items="${plans}" var="plan" varStatus="status">
                                <tr>
                                    <td>${status.count}</td>
                                    <td><fmt:formatDate value="${plan.startDate}" pattern="yyyy-MM-dd"/> 至
                                        <fmt:formatDate value="${plan.endDate}" pattern="yyyy-MM-dd"/></td>
                                    <td>${plan.content}</td>
                                </tr>
                            </c:forEach>
                            </tbody>
                            ...
                            <c:if test="${projectClose.id!=null}">
                                <c:forEach items="${proSituationList}" var="proSituation" varStatus="status">
                                    <tr>
                                        ...
                                        <td class="td_input">
                                            <textarea maxlength="2000" name="proSituationList[${status.index}].division"
                                                      required>${proSituation.division}</textarea>
                                            <br/><span class="prj-division"></span>
                                        </td>
                                        <td class="td_input">
                                            <textarea maxlength="2000"
                                                      name="proSituationList[${status.index}].situation"
                                                      required>${proSituation.situation}</textarea>
                                            <br/><span class="prj-division"></span>
                                        </td>
                                    </tr>
                                </c:forEach>
                        ...
                        <table class="table  table-hover table-bordered">
                            ...
                            <script type="text/template" id="tpl">
                                <tr id="progeressTr">
                                    ...
                                    <td class="td_input">
                                        <textarea maxlength="2000" name="proProgresseList[{{nameIdx}}].situation"
                                                  required>{{row.situation}}</textarea>
                        ...
                        <table class="table  table-hover table-bordered">
                            ...
                            <script type="text/template" id="tpl2">
                                <tr>
                                    ...
                                    <td>{{idx}}</td>
                                    <td class="td_input">
                                        <textarea maxlength="2000" name="projectCloseFundList[{{nameIdx}}].place"
                                                  required>{{row.place}}</textarea>
                                        <br/><span class="place"></span>
                                    </td>
                                    <td class="td_input">
                                        <input type="text" name="projectCloseFundList[{{nameIdx}}].count"
                                               value="{{row.count}}" maxlength="9" class="number required"/>
                                        <br/><span class="count"></span>
                                    </td>
                                    <td style="width:150px">
                                        <a href="javascript:;" onclick="addRow2('#tbody2',tpl2,'');"
                                           class="btn btn-sm btn-primary btn-sm-reset btn-add"><span
                                                class="icon-plus"></span>添加</a>
                                        {{#delBtn}}<a href="javascript:;" onclick="delRow2(this);"
                                                      class="btn btn-sm btn-primary btn-sm-reset btn-delete"><span
                                            class="icon-minus"></span>删除</a>{{/delBtn}}
                                    </td>
                                </tr>
                    ...
                    <div class="table-wrap">
                        <table class="table  table-hover table-bordered">
                            ...
                            <script type="text/template" id="tpl3">
                                <tr>
                                    <input type="hidden" name="projectCloseResultList[{{nameIdx}}].sort"
                                           value="{{idx}}"/>
                                    <td>{{idx}}</td>
                                    <td class="td_input">
                                        <textarea maxlength="2000" name="projectCloseResultList[{{nameIdx}}].result"
                                                  required>{{row.result}}</textarea>
                                        <br/><span class="result"></span>
                                    </td>
                                    <td class="td_input">
                                        <textarea maxlength="2000" name="projectCloseResultList[{{nameIdx}}].reward"
                                                  required>{{row.reward}}</textarea>
                                        <br/><span class="reward"></span>
                                    </td>
                                    <td style="width:150px">
                                        <a href="javascript:;" onclick="addRow3('#tbody3',tpl3,'');"
                                           class="btn btn-sm btn-primary btn-sm-reset btn-add"><span
                                                class="icon-plus"></span>添加</a>
                                        {{#delBtn}}<a href="javascript:;" onclick="delRow3(this);"
                                                      class="btn btn-sm btn-primary btn-sm-reset btn-delete"><span
                                            class="icon-minus"></span>删除</a>{{/delBtn}}
                                    </td>
                                </tr>
                ...
                <section class="row">
                    ...
                    <div class="textarea-wrap">
                        <%--<sys:frontFileUploadOld fileitems="${fileListMap}" filepath="proCloseForm" btnid="upload"--%>
                                             <%--readonly="true"></sys:frontFileUploadOld>--%>

                        <sys:frontFileUpload fileitems="${fileListMap}" filepath="proCloseForm"  className="accessories-h34"></sys:frontFileUpload>
                    </div>
                </section>
				<c:if test="${not empty projectClose.suggest}">
                <section class="row">
                    <div class="prj_common_info" style="height: 40px;line-height: 40px;">
                    ...
                    <textarea maxlength="2000" name="suggest"
                              class="col-xs-12 col-md-12 col-sm-12 col-lg-12 my-textarea"
                              readonly>${projectClose.suggest}</textarea>
                    <p>${projectClose.suggest}</p>
                </section>
				</c:if>
                <div class="footer-btn-wrap">
                    <%--<a href="javascript:;" class="btn btn-sm btn-primary btn-save" onclick="submit()">保存</a>--%>
                    <button type="button" data-control="uploader" class="btn btn-sm btn-primary" onclick="submit()" id="submitBtn">提交</button>
                </div>
            </div>
