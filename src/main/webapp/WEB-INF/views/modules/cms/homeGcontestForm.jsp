...
<div id="app" v-show="pageLoad" style="display: none" class="container-fluid mgb-60">
    <div class="mgb-20">
        <edit-bar :second-name="baseContentForm.id ? '修改' : '添加'"></edit-bar>
    </div>
        ...
        <el-container>
            <el-main class="main-left">
                <el-row :gutter="10">
                    <el-col :xs="24" :lg="12">
                        <el-form-item label="归属栏目：">
                            <template v-if="categoryEntries && baseContentForm.categoryId && categoryEntries[baseContentForm.categoryId]">
                            ...
                            <template v-else>
                                <p class="el-form-item-expository" style="padding-left: 0">请查看栏目管理【大赛热点】栏目路中【栏目标识】是否添加</p>
                            </template>
                ...
                <el-form-item prop="title" label="标题：">
                    <el-input v-model="baseContentForm.title" class="w300"></el-input>
                </el-form-item>
                ...
                <div class="aside-right">
                    ...
                    <el-form-item prop="video" label="视频：" label-width="80px">
                        ...
                        <div v-if="baseContentForm.video" class="video-preview" style="width: 200px; height: 100px;">
                            <video style="width: 100%; height: 100%" controls :src="baseContentForm.video | ftpHttpFilter(ftpHttp)">
                            </video>
                        </div>
                        <el-upload
                                class="upload-demo"
                    ...
                    <el-form-item prop="articlepulishDate" label="发表时间：" label-width="81px">
                        <el-date-picker v-model="baseContentForm.articlepulishDate" type="date" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期"  :picker-options="publishEpPickerOptions"
    ...
    <el-dialog title="相关推荐" :visible.sync="relateRecommendVisible"
        ...
        <recommendation v-if="relateRecommendVisible" url="/cms/cmsArticle/homeGcontestList" :related-ids="relatedIds"
    ...
    <el-dialog title="上传封面"
        ...
        <cropper-pic :img-src="uploadProPic" :disabled="isUpdating" ref="cropperPic"
                     :copper-params="{aspectRatio: 460/260}"></cropper-pic>
        <div slot="footer" class="dialog-footer">
...
<script>
    ...
    new Vue({
        ...
        data: function () {
            ...
            var relatedIds = [],videoList = [];
            relatedIds = cmsArticleData.relation ?  cmsArticleData.relation.split(',') : [];
            videoList = cmsArticleForm.video ? [{ftpUrl:cmsArticleForm.video}] : [];
            return {
                ...
                baseContentForm: {
                    ...
                    isshowlink: cmsArticleForm.isshowlinkt || '0',
                    cmsArticleData: {
...
//                        module: [{required: true, message: '请选择栏目模块', trigger: 'change'}],
        ...
        methods: {
            ...
            getCategoryList: function () {
                var self = this;
                this.$axios.get('/cms/category/cmsCategoryList?publishCategory=0000000285').then(function (response) {
...
//                            self.setFlattenMenuList([].concat(self.menuList))
            ...
            saveForm:function (formName) {
                ...
                this.$refs[formName].validate(function (valid) {
                    if(valid){
                        ...
                        self.$axios({
                            method:'POST',
                            url:'/cms/cmsArticle/homeGcontestSave',
                            ...
                            if (checkResponseCode) {
                                self.$msgbox({
                                    ...
                                    location.href = self.frontOrAdmin + '/gcontesthots/list'
                                }).catch(function () {
            ...
            uploadVideoSuccess: function (response, file, fileList) {
                ...
                this.$message({
                    type:  response.state === 'SUCCESS' ? 'success' : 'error',
                    message: response.state === 'SUCCESS' ? '上传成功' : '上传失败'
                })
            },
