...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/oa/oaNotify/assignList">定向通知列表</a></li>
		<li><a href="${ctx}/oa/oaNotify/formAssign?id=${oaNotify.id}">定向通知<shiro:hasPermission
	...
	<form:form id="searchForm" modelAttribute="oaNotify"
		action="${ctx}/oa/oaNotify/assignList" method="post"
	...
	<table id="contentTable"
		...
		<tbody>
			<c:forEach items="${page.list}" var="oaNotify">
				<tr>
					<td><a href="${ctx}/oa/oaNotify/formAssign?id=${oaNotify.id}">
					...
					<%-- <td>
					<fmt:formatDate value="${oaNotify.effectiveDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${oaNotify.endDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td> --%>
					<%-- <c:if test="${!requestScope.oaNotify.self}"> --%>
						<shiro:hasPermission name="oa:oaNotify:edit">
							<td><a
								href="${ctx}/oa/oaNotify/formAssign?id=${oaNotify.id}">修改</a> <a
								href="${ctx}/oa/oaNotify/delete?id=${oaNotify.id}"
