...
<div class="container-fluid">
    <%--<div class="edit-bar clearfix">--%>
        <%--<div class="edit-bar-left">--%>
            <%--<span><shiro:hasPermission name="sys:role:edit">角色分配</shiro:hasPermission><shiro:lacksPermission--%>
                    <%--name="sys:role:edit">人员列表</shiro:lacksPermission></span>--%>
    ...
    <div class="clearfix mgb-20 pull-right">
        <button id="assignButton" class="btn btn-primary" type="submit">分配角色</button>
        <button class="btn btn-default" type="button" onclick="history.go(-1)">返回</button>
    </div>
    <script type="text/javascript">
        $("#assignButton").click(function () {
            top.$.jBox.open("iframe:${ctx}/sys/role/usertorole?id=${role.id}", "分配角色", 570, $(top.document).height() - 240, {
    ...
    <table id="contentTable" class="table table-bordered table-condensed table-hover table-center table-orange">
        ...
        <c:forEach items="${userList}" var="user">
            <tr>
                ...
                <td>${user.office.name ? user.office.name : '-'}</td>
