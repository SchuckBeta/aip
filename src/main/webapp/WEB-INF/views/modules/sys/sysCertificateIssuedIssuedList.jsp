...
<body>
	...
	<div class="content_panel">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="${ctx}/sys/sysCertificateIssued/issuedList">证书颁布记录列表</a></li>
			<li><a href="${ctx}/sys/sysCertificateIssued/ywList">证书业务记录列表</a></li>
			<%-- <shiro:hasPermission name="sys:sysCertificateIssued:edit"><li><a href="${ctx}/sys/sysCertificateIssued/form">证书颁布记录添加</a></li></shiro:hasPermission> --%>
		</ul>
		<form:form id="searchForm" modelAttribute="sysCertificateIssued" action="${ctx}/sys/sysCertificateIssued/issuedList" method="post" class="breadcrumb form-search">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed table-thead-bg">
			...
			<c:forEach items="${page.list}" var="sysCertificateIssued">
				<tr>
					...
					<td>${sysCertificateIssued.issuedBy.name}</td>
					<td>
					...
					<%-- <shiro:hasPermission name="sys:sysCertificateIssued:edit"><td>
						...
						<a class="check_btn btn-pray btn-lx-primary" href="${ctx}/sys/sysCertificateIssued/delete?id=${sysCertificateIssued.id}" onclick="return confirmx('确认要删除该证书颁布记录吗？', this.href)">删除</a>
