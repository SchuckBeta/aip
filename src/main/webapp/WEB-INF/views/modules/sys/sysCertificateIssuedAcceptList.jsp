...
<body>
	...
	<div class="content_panel">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="${ctx}/sys/sysCertificateIssued/acceptList">证书获取记录列表</a></li>
			<li><a href="${ctx}/sys/sysCertificateIssued/issuedList">证书颁布记录列表</a></li>
			<li><a href="${ctx}/sys/sysCertificateIssued/ywList">证书业务记录列表</a></li>
			<%-- <shiro:hasPermission name="sys:sysCertificateIssued:edit"><li><a href="${ctx}/sys/sysCertificateIssued/form">证书获取记录添加</a></li></shiro:hasPermission> --%>
		</ul>
		<form:form id="searchForm" modelAttribute="sysCertificateIssued" action="${ctx}/sys/sysCertificateIssued/acceptList" method="post" class="breadcrumb form-search">
			<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
			<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			<ul class="ul-form">
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/></li>
				<li><label>被执行人：</label>
					<sys:treeselect id="userIssuedBy" name="acceptBy.id" value="${sysCertificateIssued.acceptBy.id}" labelName="acceptBy.name" labelValue="${sysCertificateIssued.acceptBy.name}"
					title="执行用户" url="/sys/office/treeData?type=3" cssClass="" allowClear="true" notAllowSelectParent="true"/>
				</li>
				<li><label>执行人：</label>
					<sys:treeselect id="userAcceptBy" name="issuedBy.id" value="${sysCertificateIssued.issuedBy.id}" labelName="issuedBy.name" labelValue="${sysCertificateIssued.issuedBy.name}"
					title="执行用户" url="/sys/office/treeData?type=3" cssClass="" allowClear="true" notAllowSelectParent="true"/>
				</li>
				<li><label>证书：</label>
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed table-thead-bg">
			...
			<c:forEach items="${page.list}" var="sysCertificateIssued">
				<tr>
					...
					<td>${sysCertificateIssued.acceptBy.name}</td>
					<td>
					...
					<%-- <shiro:hasPermission name="sys:sysCertificateIssued:edit"><td>
						...
						<a class="check_btn btn-pray btn-lx-primary" href="${ctx}/sys/sysCertificateIssued/delete?id=${sysCertificateIssued.id}" onclick="return confirmx('确认要删除该证书获取记录吗？', this.href)">删除</a>
