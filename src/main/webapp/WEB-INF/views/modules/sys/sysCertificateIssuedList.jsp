...
<body>
	...
	<div class="content_panel">
		<ul class="nav nav-tabs">
			<li class="active"><a href="${ctx}/sys/sysCertificateIssued/list">证书记录列表</a></li>
			...
			<%-- <shiro:hasPermission name="sys:sysCertificateIssued:edit"><li><a href="${ctx}/sys/sysCertificateIssued/form">证书记录添加</a></li></shiro:hasPermission> --%>
		</ul>
		<form:form id="searchForm" modelAttribute="sysCertificateIssued" action="${ctx}/sys/sysCertificateIssued/list" method="post" class="breadcrumb form-search">
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed table-thead-bg">
			...
			<c:forEach items="${page.list}" var="sysCertificateIssued">
				<tr>
					...
					<td>${sysCertificateIssued.sysCertRel.sysCert.name}</td>
					<td>
					...
					<%-- <shiro:hasPermission name="sys:sysCertificateIssued:edit"><td>
						...
						<a class="check_btn btn-pray btn-lx-primary" href="${ctx}/sys/sysCertificateIssued/delete?id=${sysCertificateIssued.id}" onclick="return confirmx('确认要删除该证书记录吗？', this.href)">删除</a>
