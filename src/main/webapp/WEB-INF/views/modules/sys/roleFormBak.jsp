...
<div class="container-fluid">
    <%--<div class="edit-bar clearfix">--%>
        <%--<div class="edit-bar-left">--%>
            <%--<span>角色<shiro:hasPermission--%>
                    <%--name="sys:role:edit">${not empty role.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission--%>
                    <%--name="sys:role:edit">查看</shiro:lacksPermission></span>--%>
    ...
    <form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
        ...
        <%--<div class="control-group">--%>
            ...
            <%--<div class="controls">--%>
                <%--<sys:treeselect id="office" name="office.id" value="${role.office.id}" labelName="office.name"--%>
                                ...
                                <%--title="机构" url="/sys/office/treeData" cssClass="required" cssStyle="width: 175px;"/>--%>
            <%--</div>--%>
        <%--</div>--%>
        <div class="control-group">
            <label class="control-label"><i>*</i>角色名称：</label>
            <div class="controls">
				...
				<div class="control-group">
            <label class="control-label"><i>*</i>角色业务类型：</label>
            <div class="controls">
                <form:select path="bizType" class="required">
                	<form:option value="">--请选择--</form:option>
                    <form:options items="${fns:getDictList('0000000153')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </div>
        </div>
        <div class="control-group">
            <label class="control-label"><i>*</i>英文名称：</label>
            <div class="controls">
                ...
                <span class="help-inline gray-color">工作流用户组标识</span>
            </div>
        </div>
        <%--<div class="control-group">--%>
            <%--<label class="control-label">用户组类型：</label>--%>
            <%--<div class="controls">&lt;%&ndash;--%>
                ...
                <%--</form:select>--%>
                <%--<span class="help-inline gray-color"--%>
        ...
        <%--</div>--%>
        <div class="control-group">
            <label class="control-label">角色范围：</label>
            <div class="controls"><%--
                <form:input path="roleType" htmlEscape="false" maxlength="50" class="required"/>
                <span class="help-inline" title="activiti有3种预定义的组类型：security-role、assignment、user 如果使用Activiti Explorer，需要security-role才能看到manage页签，需要assignment才能claim任务">
                    工作流组用户组类型（security-role：管理员、assignment：可进行任务分配、user：普通用户）</span> --%>
                <form:select path="roleGroup">
                    <form:option value="1">前台</form:option>
                    <form:option value="2">后台</form:option>
                </form:select>
                <span class="help-inline gray-color" >
                    角色范围（前台：登录前台展示页面 后台：登录后台管理页面）</span>
            </div>
        </div>
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <span class="help-inline gray-color">“是”代表此数据只有超级管理员能进行修改，“否”则表示拥有角色修改人员的权限都能进行修改</span>
            </div>
        </div>
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <span class="help-inline gray-color">“是”代表此数据可用，“否”则表示此数据不可用</span>
            </div>
        </div>
        <c:if test="${admin}">
        <div class="control-group">
        ...
        <div class="form-actions">
            <c:if test="${admin or(role.id !='1' and role.id !='10')}">
