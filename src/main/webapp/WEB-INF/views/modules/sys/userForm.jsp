...
//            }
            var hasStu=false;
            var hasTea=false;
            $("input[name='roleIdList']:checked").each(function (i, v) {
                var biztype=rolesjs[$(v).val()];
                if(biztype=='1'){
                    hasStu=true;
                }
                if(biztype=='2'){
            ...
            if(!hasStu&&hasTea){
                $("#teacherTypeDiv").attr("style","display:");
            }else{
                $("#teacherTypeDiv").attr("style","display:none");
            }
        }
    </script>
...
<div class="container-fluid">
    ...
    <form:form id="inputForm" modelAttribute="user"
               action="${ctx}/sys/user/save" method="post" class="form-horizontal" autocomplete="off">
        <form:hidden path="id"/>
        ...
        <div class="control-group">
            ...
            <div class="controls controls-checkbox">
                <form:checkboxes path="roleIdList" items="${allRoles}" onclick="checkRoles(this)"
                                 itemLabel="name" itemValue="id" htmlEscape="false"
                                 class="required"/>
            </div>
        </div>
        <c:choose>
            <c:when test="${fns:checkHasRoleBizType(user,'2')}">
                <div class="control-group"  id="teacherTypeDiv"  >
            ...
            <c:otherwise>
                <div class="control-group" id="teacherTypeDiv"  style="display:none" >
                    <label class="control-label">导师来源:</label>
                    <div class="controls">
                        <form:select path="teacherType">
                            <form:options items="${fns:getDictList('master_type')}"
        ...
        <div class="form-actions">
            ...
            <a class="btn btn-default" href="${ctx}/sys/user/list">返回</a>
        </div>
    </form:form>
</div>
