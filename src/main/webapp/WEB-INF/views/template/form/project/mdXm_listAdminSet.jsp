...
<head>
    ...
    <script type="text/javascript">
        ...
        function impData() {
            location.href = "/a/impdata/mdlist?type=6&actywId=${actywId}&gnodeId=${gnodeId}";
        }
    ...
    <form:form id="searchForm" modelAttribute="act" action="${actionUrl}" method="post"
        ...
        <shiro:hasPermission name="sys:user:import">
            <div class="text-right mgb-20">
                <button class="btn btn-primary expbtn" disabled type="button">导出</button>
    ...
    <table id="contentTable"
        ...
        <c:forEach items="${page.list}" var="proModelMd">
            ...
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${proModelMd.setState!=null &&proModelMd.setState=='1'}">
                            通过
                        </c:when>
                        <c:when test="${proModelMd.setState!=null &&proModelMd.setState=='0'}">
                            不通过
                        </c:when>
                        <c:otherwise>
                            <a href="${ctx}/actyw/actYwGnode/designView?groupId=${auditGonde.group.id}&proInsId=${proModelMd.proModel.procInsId}"
                               target="_blank">待审核</a>
                        </c:otherwise>
                    </c:choose>
                </td>
                    ...
                    <%--<a class="btn btn-primary btn-small" href="${ctx}/promodel/proModel/viewForm?id=${proModelMd.proModel.id}">查看</a>--%>
                    <%--  <a class="btn btn-back-oe btn-primaryBack-oe btn-small"
                         href="${ctx}/act/task/auditform?proModelId=${proModelMd.proModel.id}&path=${path}&pathUrl=${actionUrl}">查看</a>
                      --%><%--</c:if>--%>
                    <%--<c:if test="${act.status=='finish'}">
                       	<a href="${ctx}/actyw/actYwGnode/designView?groupId=${auditGonde.group.id}&proInsId=${proModelMd.proModel.procInsId}" class="check_btn btn-pray btn-lx-primary" target="_blank">查看</a>
                    </c:if>--%>
                    <%--</td>--%>
            </tr>
        </c:forEach>
