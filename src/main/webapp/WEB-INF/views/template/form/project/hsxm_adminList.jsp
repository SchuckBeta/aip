...
<div id="app" v-show="pageLoad" style="display: none" class="container-fluid mgb-60">
    ...
    <el-form :model="searchListForm" size="mini" autocomplete="off" ref="searchListForm">
        ...
        <div class="search-block_bar clearfix">
            <div class="search-btns">
                ...
                <el-button v-if="isAdmin != null && isAdmin =='1' && isGate != null && isGate =='1'" type="primary" :disabled="projectListMultipleSelection.length < 1" size="mini"
                           @click.stop.prevent="batchPass">
    ...
    <div class="table-container" style="margin-bottom:40px;">
        <el-table size="mini" :data="projectListFlatten" class="table" ref="multipleTable" v-loading="loading"
            ...
            <el-table-column v-if="isScore != null && isScore == '1'" label="评分" align="center" prop="gScore" min-width="85" sortable>
                <template slot-scope="scope">
                    {{scope.row.gScore}}
                </template>
            </el-table-column>
...
<script>
    ...
    new Vue({
        ...
        methods: {
            ...
            batchPass: function () {
                var self = this;
                this.$confirm('确定批量审核通过？', '批量通过', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(function () {
                    self.loading = true;
                    var passXhr = self.$axios({
                        method: 'POST',
                        url: '/promodel/proModel/ajax/batchAudit?ids=' + self.multipleSelectedId.join(',') + '&grade=' + '${actYwStatus.status}' + '&gnodeId=' + '${gnodeId}',
                        timeout: 1000* 60 * 30,
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'
                        }
                    });
                    passXhr.then(function (response) {
                        var data = response.data;
                        if (data.ret == '1') {
