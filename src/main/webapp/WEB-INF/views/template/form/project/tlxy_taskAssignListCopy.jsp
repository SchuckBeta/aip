...
<div class="container-fluid">
    ...
    <form:form id="searchForm" modelAttribute="proModelTlxy" action="${actionUrl}" method="post"
        ...
        <div class="col-control-group">
        	<div class="control-group">
                ...
                <div class="controls">
                    <form:select path="proModel.gnodeId" class="input-medium">
            ...
            <div class="control-group" style="margin-right: 400px;">
                ...
                <div class="controls">
                    <select class="input-medium form-control" name="hasAssigns" >
								...
								<option value="0" <c:if test="${proModel.hasAssigns =='0'}">selected</c:if> >未指派</option>
								<option value="1" <c:if test="${proModel.hasAssigns =='1'}">selected</c:if> >已指派</option>
							</select>
                </div>
            </div>
        </div>
    ...
    <div id="auditInfo">
        <table id="contentTable"
            ...
            <c:forEach items="${page.list}" var="item">
                ...
                <tr>
                	<td><input type="checkbox" name="subck" onclick="subckchange(this)" value="${item.proModel.id}"></td>
                    <td>${item.proModel.competitionNumber}</td>
                    <td><a href="${ctx}/promodel/proModel/viewForm?id=${item.id}">${item.proModel.pName}</a></td>
                    <td>${fns:getDictLabel(item.proModel.proCategory, "project_type", "")}</td>
...
function subckchange(ob) {
    ...
    function toTaskAssign(){
        var gnodeId = $("[id='proModel.gnodeId']").val();
    	if (!gnodeId) {
            alertx("请选择要指派的节点");
            return;
        }
