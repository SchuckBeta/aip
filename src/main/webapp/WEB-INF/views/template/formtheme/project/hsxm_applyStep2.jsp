    ...
    <div class="main-wrap">
        ...
        <div class="row-apply">
            ...
            <form:form id="form1" modelAttribute="proModelHsxm" action="#" method="post" class="form-horizontal"
                ...
                <input type="hidden" name="proModel.year" id="proModel.year" value="${proModelHsxm.proModel.year}"/>
                <input type="hidden" name="proModel.id" id="proModel.id" value="${proModelHsxm.proModel.id}"/>
                            ...
                            <table class="studenttb table table-bordered table-condensed table-orange table-center table-nowrap table-hover">
                                ...
                                <c:if test="${teamStu!=null&&teamStu.size() > 0}">
                                    <c:forEach items="${teamStu}" var="item" varStatus="status">
                                        <tr>
                                            <td>${status.index+1 }
                                                <input type="hidden" name="proModel.studentList[${status.index}].userId"
...
//        });
    ...
    function saveStep2(obj) {
        if (tlxyValidate.form()) {
            ...
            $("#form1").attr("action", "/f/proModelHsxm/ajaxSave2");
            ...
            $("#form1").ajaxSubmit(function (data) {
                if (checkIsToLogin(data)) {
                    ...
                    if (data.ret == 1) {
                        top.location = "/f/proModelHsxm/applyStep3?id=" + data.id;
                    } else {
    ...
    function toPrevStep(id) {
        top.location = "/f/proModelHsxm/applyStep1?actywId=" + $("#actYwId").val() + (id == "" ? "" : "&id=" + id);
    }
    function saveStep2UnCheck(type) {
        ...
        $("#form1").attr("action", "/f/proModelHsxm/ajaxUncheckSave2");
        $("#form1").ajaxSubmit(function (data) {
            if (checkIsToLogin(data)) {
                dialogCyjd.createDialog(0, "未登录或登录超时。请重新登录，谢谢！", {
                    dialogClass: 'dialog-cyjd-container none-close',
