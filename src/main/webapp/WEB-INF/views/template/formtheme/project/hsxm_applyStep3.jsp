    ...
    <div class="main-wrap">
        ...
        <div class="row-apply">
            ...
            <form:form id="form1" modelAttribute="proModelHsxm" action="#" method="post" class="form-horizontal"
                ...
                <input type="hidden" name="actYwId" id="actYwId" value="${proModelHsxm.proModel.actYwId}"/>
                <input type="hidden" name="proModel.proType" id="id" value="${proModelHsxm.proModel.proType}"/>
                ...
                <div class="form-horizontal form-enter-apply" style="margin-left:95px;">
                    <div class="form-group">
                        ...
                        <div class="col-xs-8">
                            <c:if test="${proModelHsxm.proModel.subStatus=='1'}">
                                <sys:frontFileUpload className="accessories-h34" readonly="true" fileitems="${proModelHsxm.proModel.fileInfo}" filepath="project" tip1="支持上传word、pdf、excel、txt、rar、ppt图片类型等文件" tip2="（上传项目申报表单、商业计划书等项目文件）"></sys:frontFileUpload>
                            </c:if>
                            <c:if test="${proModelHsxm.proModel.subStatus!='1'}">
                                <sys:frontFileUpload className="accessories-h34"  fileitems="${proModelHsxm.proModel.fileInfo}" filepath="project" tip1="支持上传word、pdf、excel、txt、rar、ppt图片类型等文件" tip2="（上传项目申报表单、商业计划书等项目文件）"></sys:frontFileUpload>
                            </c:if>
...
<script>
    function saveStep3(obj) {
        ...
        $("#form1").attr("action", "/f/proModelHsxm/ajaxSave3");
                ...
                buttons: [
                    {
                        ...
                        click: function () {
                            ...
                            var onclickFn = $(obj).attr("onclick");
                            $(obj).removeAttr("onclick");
                            $("#form1").attr("action", "/f/proModelHsxm/submit");
                            $(obj).prop('disabled', true);
                            $("#form1").ajaxSubmit(function (data) {
                                if (checkIsToLogin(data)) {
                                    dialogCyjd.createDialog(data.ret, "未登录或登录超时。请重新登录，谢谢！", {
...
//                                $(obj).prop('disabled', false);
    ...
    function toPrevStep(id){
        top.location="/f/proModelHsxm/applyStep2?id="+id;
    }
    function saveStep3UnCheck(){
        ...
        $("#form1").attr("action","/f/proModelHsxm/saveStep3");
        $("#form1").ajaxSubmit(function (data) {
            if(checkIsToLogin(data)){
                dialogCyjd.createDialog(data.ret, "未登录或登录超时。请重新登录，谢谢！", {
                    dialogClass: 'dialog-cyjd-container',
