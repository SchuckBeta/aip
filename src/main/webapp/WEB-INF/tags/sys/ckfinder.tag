...
<ol id="${input}Preview" style="padding: 0; margin: 0;list-style: none"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn btn-primary btn-small">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn btn-default btn-small">清除</a></c:if>
