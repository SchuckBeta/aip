...
<script type="text/x-template" id="flowNodeModalTemplate">
    <div class="modal-module" :data-show="controlModal">
        <my-modal :title="title" :show.sync="modalShow" @ok="saveNode" @cancel="cancel" :save-disabled="saveDisabled"
            ...
            <div class="modal-body" slot="body">
                <form class="form-horizontal">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="hidden" v-model="nodeData.groupId"/>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="hidden" v-model="nodeData.parent.id"/>
                    ...
                    <div v-show="hidePreFunIds" class="control-group">
                        ...
                        <div class="controls">
                            <select v-model="nodeData.preFunId">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" class="form-control input-large" v-model="nodeData.name"
                                   @change="hasNodeGroupName = !nodeData.name"/>
                    ...
                    <div v-show="hideNextFunIds" class="control-group">
                        ...
                        <div class="controls">
                            <select v-model="nodeData.nextFunId" class="form-control input-large">
                                <option :value="0">请选择</option>
                                <option v-for="(item, index) in nextNodes" :value="item.id"
                                        v-option-bind="{label: item.name}"></option>
                            </select>
                        </div>
                    </div>
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <select v-model="nodeData.formId" class="form-control input-large">
                                <option :value="0">请选择</option>
                                <option v-for="(item, index) in forms" :value="item.id" :data-list-id="item.listId" :ref="'option'+ item.id"
                                        v-option-bind="{label: item.name}"></option>
                            </select>
                        </div>
                    </div>
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <div class="img-content" style="max-width: 100px; max-height: 100px;line-height: 1">
                                <img :id="imgId" ref="iconPic" :src="nodeData.iconImgUrl"
                                     style="display: block;max-width: 100%"/>
                            </div>
                            <%--<input type="hidden" name="file" v-model="nodeGroupForm.iconUrl"/>--%>
                            <input type="button" @click="choosePic = true" class="btn" value="更新图标"/>
                            <div v-show="controlIconUrl" class="red">
                                请上传图标
                            </div>
                        </div>
                    </div>
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <label><input type="radio" v-model="nodeData.isShow" :value="1"
                                          class="form-control"/>是</label>
                            <label><input type="radio" v-model="nodeData.isShow" :value="0"
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <select v-model="nodeData.flowGroup" class="form-control input-large">
                                <option :value="0">所有角色</option>
                                <option v-for="(item, index) in roles" v-option-bind="{label: item.name}"
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea class="form-control input-xlarge" v-model="nodeData.remarks"></textarea>
                        </div>
                    </div>
                </form>
            </div>
        </my-modal>
        <my-modal title="选择图片" :class="modalPicClass" :show.sync="choosePic" @ok="saveNode" @cancel="choosePic = false"
                  :has-save="false" ok-text="确定" cancel-text="取消" :large="true">
            <div class="modal-body" slot="body">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane" :class="{active: !defaultImgTab}">
                        <div class="upload-content"
                             v-upload-file="{src: nodeData.iconImgUrl, cropOption: cropOption, url: severUrl,fileVal: fileVal}">
                            上传本地图标
                        </div>
                    </div>
                </div>
...
<script>
    var flowNodeModal = Vue.component('flow-node-modal', {
        template: '#flowNodeModalTemplate',
        components: {},
        props: {
            iconList: {
                type: Array,
        ...
        directives: {
            optionBind: {
                bind: function (el, binding) {
                    ...
                    el.innerHTML = label.indexOf('/') ? label.substring(label.indexOf('/') + 1) : label
                }
            },
            uploadFile: {
                twoWay: true,
...
//
                    ...
                    $btnUpload.on('click', function () {
                        ...
                        oReq.onload = function (oEvent) {
                            if (oReq.status == 200) {
                                vnode.context.nodeData.iconImgUrl = JSON.parse(oReq.responseText).url;
                                vnode.context.nodeData.iconUrl = JSON.parse(oReq.responseText).ftpUrl;
        ...
        watch: {
            ...
            flowType: function () {
                if (this.modalType === 'flowNodeGroup') {
                    this.getForms();
                    this.getRoles();
                }
            },
        ...
        methods: {
            ...
            getRoles: function () {
                var xhr = $.get('/a/actyw/gnode/ajaxGnodeRole');
                var self = this;
            ...
            getForms: function () {
                var xhr = $.get('/a/actyw/gnode/ajaxGnodeForm/' + this.flowType + '?proType=' + this.proType);
                var self = this;
                xhr.success(function (data) {
                    self.forms = data
                })
            ...
            chooseDefaultImg: function (src, event) {
                ...
                var input = '<input name="file" type="hidden" value="' + src + '">';
                this.nodeData.iconImgUrl = $(event.target).attr('src');
                this.nodeData.iconUrl = src;
