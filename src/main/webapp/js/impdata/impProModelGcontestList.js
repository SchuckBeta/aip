$(document).ready(function() {
	...
	$('#fileToUpload').on('change', function() {
	    $.ajaxFileUpload({
	        url:'/a/impdata/importProModelGcontestData?actywId=' + $("#actywId").val() + '&gnodeId=' + $("#gnodeId").val() + "&hasFile=1",
	        secureuri:false,
...
function downTemplate(){
	var href = "/a/impdata/downProModelGcontestTemplate?1=1";
	if($("#actywId").val()){
...
function getImpInfo(){
	$("[uncomplete]").each(function(){
		...
		getImpInfoSub(id);
		eval("it"+id+"=setInterval(function(){getImpInfoSub(id);},5000);");
	});
}
