...
//            $modalAvatarArea.find('>img').cropper('destroy');
    ...
    $avatarFile.on('change', function (event) {
        ...
        var rFilter = /^(?:image\/jpeg|image\/jpeg|image\/jpeg|image\/png|image\/PNG|image\/jpg|image\/JPG)$/i;
        oFReader = new FileReader();
        if (aFiles[0].type && !rFilter.test(aFiles[0].type)) {
            event.target.value = '';
            alertx('上传图片格式不正确');
            return false;
        }
    ...
    $btnModalUpload.on('click', function () {
        ...
        oReq.onload = function (oEvent) {
            if (oReq.status == 200) {
                ...
                $modal.trigger(successEvent, [JSON.parse(oReq.responseText), roundedCanvas.toDataURL()]);
                $modal.modal('hide');
            } else {
