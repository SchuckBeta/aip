...
Vue.component('mobile-form', {
    ...
    computed: {
        ...
        mobileRule: function () {
            var self = this;
            var validateRepeatMobile = function (rules, value, callback) {
                if (value) {
                    if (value === self.oldMobile) {
                        return callback(new Error("请不要输入已绑定的手机号"))
                    }
                    return callback();
                }
                return callback();
            ...
            var rules = [
                ...
                {validator: validateRepeatMobile, trigger: 'blur'}
            ];
